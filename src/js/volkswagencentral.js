...
const SLIDER_HERO = [
  {
    ...
    description:
      "See each Atlas Cross Sport side by side and compare trims to build your perfect match. Want to get started? Select a trim below.",
  },
  {
    background: "#103369",
    image:
    ...
    title: "Golf R. Golf R",
    description:
      "Golf R with 4MOTION® 2.0 TSI 7-speed DSG® automatic transmission with Tiptronic® and Sport mode",
  },
  {
    background: "#fff",
    image:
      "https://assets.volkswagen.com/is/image/volkswagenag/VW_NGW6_Models_ID-Buzz_v2?Zml0PWNyb3AsMSZmbXQ9d2VicC1hbHBoYSZxbHQ9Nzkmd2lkPTUwMCZiZmM9b2ZmJjkzMDk=",
  ...
  },
  {
    ...
    title: "Pro S",
    description:
      "MacPherson strut-type with lower control arm, coil springs, telescopic dampers, anti-roll bar",
  },
  {
    background: "#fff",
    image:
      "https://assets.volkswagen.com/is/image/volkswagenag/Polo_1920x1080-5?Zml0PWNyb3AlMkMxJndpZD0xOTIwJmhlaT0xMDgwJmZtdD1wbmctYWxwaGEmYmZjPW9mZiY1M2Qx",
    title: "The Polo",
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .querySelectorAll('[data-component="hero-banner"]')
    .forEach(($slider) => {
      new Slider($slider);
    });
  ...
  document
    .querySelectorAll('[data-component="video-player"]')
    .forEach(($videoPlayer) => {
...
class ContentPage {
  ...
  reactiveData() {
    ...
    blocks.forEach((data) => {
      const $sliderHero = div({
        attrs: {
          class: 'slider-hero',
          'data-component': 'slider-hero',
        },
        childs: [
          {
                ...
                childs: [
                  {
                    ...
                    attrs: {
                      'data-title': '',
                      class: 'animate in-view font-size:50px margin:0 @media(max-width:768px):font-size:32px color:#fff font-weight:bold'
                    },
                    childs: data?.items?.[0]?.title || ''
                  },
                  {
                    name: 'p',
                    attrs: {
                      'data-description': '',
                      class: 'in-view animate font-size:18px color:#fff line-height:1.8'
                    },
                    childs: data?.items?.[0]?.description || ''
                  }
                ]
              },
              {
            ...
            childs: [
              {
                ...
                childs: [
                  { attrs: { class: 'navigation-prev-inner navigation-item-inner' } },
                  { attrs: { class: 'navigation-prev-inner-overlay navigation-item-overlay' } }
                ]
              },
              {
                attrs: {
      ...
      this.$element.append($sliderHero);
...
// const contentPage = new ContentArea();

// contentArea.injectContent(({ getContent }) => {
//   // // 1. Slider Hero
//   // document.querySelectorAll('[data-component="slider-hero"]').forEach(($sliderHero) => {
//   //   const content = getContent('sliderHero');
//   //   new SliderHero($sliderHero, content.map(item => {
//   //     return {
//   //       background: item.fields.background,
//   //       image: item.fields.image.asset.fields.file.url,
...
//   // document.querySelectorAll('[data-component="hero-banner-content"]').forEach(($sliderHeroContent) => {
//   //   const products = getContent('product');
//   //   console.log('products', products);

//   //   $sliderHeroContent.innerHTML = products.map(({ fields }) => {
//   //     return `
//   // 			  <div>
//   //           <img class="width:100%"
//   //             src="${fields.image}" alt=""
//   //             loading="lazy" />
//   //           <div class="text-align:center font-size:16px">${fields.title}</div>
//   //         </div>
//   // 		`
//   //   }).join('');

//   //   new Slider($sliderHeroContent);

//   // });
// });

// Array.from(res.items).map(item => {
//   let { fields } = item;
//   Object.entries(fields).forEach(([key, value]) => {
//     const hasMany = isArray(value);
//     const hasOne = isObject(value)
//     if (hasMany) {
//       fields[key] = value.map((val) => {
//         const id = val.sys.id;
//         const find = res.items.find((item) => item.sys.id == id);
//         return find.fields;
//       });
//     } else if (hasOne) {
//       const id = value?.sys?.id;
//       const find = res.includes.Asset.find((item) => item.sys.id == id);
//       fields[key] = find?.fields;
...
const div = ({ name = 'div', attrs, childs }) => {
