...
class SliderHero {
	...
	updateView() {
		...
		const { background, image, title, description } = this.contentFields[this.index];
		...
		const updateBackground = () => {
			...
			this.$element.querySelector('[data-navigation-prev] .navigation-item-overlay').style.backgroundColor = this.contentFields[prevIndex].background;
			this.$element.querySelector('[data-navigation-next] .navigation-item-overlay').style.backgroundColor = this.contentFields[nextIndex].background;
...
class ContentArea {
	...
	async fetchData() {
		return new Promise((resolve, reject) => {
			const apiUrl = 'https://cdn.contentful.com/spaces/wxetfbdill9l/environments/master/entries?access_token=AEIaSxjAus-7NLAcPmYrWZNWDCOOwFapmleWjBqr6e0';
			if (window.localStorage.getItem(apiUrl) && false) {
				this.data = JSON.parse(window.localStorage.getItem(apiUrl));
				resolve(this.data);
			} else {
				return fetch(apiUrl)
	...
	reactiveData() {
		this.injectCallbacks.forEach((key, callback) => {
			callback(this);
		});
	}
	...
	getContent(contentTypeId) {
		const { items, includes } = this.data;
		return items.filter(item => item.sys.contentType.sys.id === contentTypeId).map(item => {
			const data = { ...item };
			const { fields } = item;

			Object.entries(fields).forEach(([key, value]) => {
				if (typeof value === 'object' && value?.sys?.linkType === 'Asset') {
					data.fields[key] = {
						...data.fields[key],
						asset: includes.Asset.find(asset => asset.sys.id === value.sys.id),
					}
				}
			});
			return data;
...
contentArea.injectContent(({ getContent }) => {
	...
	document.querySelectorAll('[data-component="slider-hero-content"]').forEach(($sliderHeroContent) => {
		const products = getContent('productSlider');
		$sliderHeroContent.innerHTML = products.map(item => {
			return `
				<div>
					<img src="${item.fields.image.asset.fields.file.url}" alt="${item.fields.title}">
				</div>
			`
		}).join('');
